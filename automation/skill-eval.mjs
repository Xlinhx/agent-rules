...
for (const skill of implicit) {
  ...
  const measured = measureSkill(folder, body.length);
  rows.push({
    id: skill.id,
    description_chars: description.length,
    body_tokens: measured.skill_body_tokens,
    positives: cases.filter((entry) => entry.kind === 'positive').length,
    hard_negatives: cases.filter((entry) => entry.kind === 'hard_negative').length,
    cross_language: cases.filter((entry) => entry.language && entry.language !== 'en').length,
    body_reference_tokens: measured.body_reference_tokens,
    script_text_tokens: measured.script_text_tokens,
    supporting_text_tokens: measured.supporting_text_tokens,
    text_loadable_files: measured.text_loadable_files,
    binary_asset_files: measured.binary_asset_files,
    binary_asset_bytes: measured.binary_asset_bytes,
  });
}
...
for (const skill of (registry.skills ?? []).filter((entry) => entry.lifecycle === 'active' && entry.activation === 'explicit-only')) {
  ...
  explicitRows.push({
    ...
    skill_body_tokens: measured.skill_body_tokens,
    reference_tokens: measured.reference_tokens,
    body_reference_tokens: measured.body_reference_tokens,
    script_text_tokens: measured.script_text_tokens,
    supporting_text_tokens: measured.supporting_text_tokens,
    text_loadable_files: measured.text_loadable_files,
