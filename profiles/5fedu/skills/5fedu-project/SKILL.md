...
This skill is eligible only when the host/runtime has already activated the 5fedu project scope, for example `domain_pack: 5fedu`, `--profile 5fedu`, or the explicit compatibility marker. Words such as `drawer`, `ERP`, `listview`, or `5fedu` in a prompt are not sufficient activation by themselves.
