...
A target project does not need the template or shared pack copied into it. Activate `domain_pack: 5fedu` (or `--profile 5fedu`) and read template code through the central `reference` / `reference --search` broker.
